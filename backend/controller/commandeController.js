...
exports.createCommande = async (req, res) => {
    try {
        const { client, adresse, telephone, methodePaiement, products, etat, clientEmail } = req.body;
        ...
        for (const item of products) {
            ...
            if (produit.stockQuantity < item.stockQuantity) {
                return res.status(400).json({ message: `Quantité insuffisante pour le produit: ${produit.name}` });
            }
            total += produit.price * item.stockQuantity;
        }

        // Créer la commande avec le montant total
        const newCommande = new Commande({ client, adresse, telephone, methodePaiement, etat, products, total });
        const savedCommande = await newCommande.save();

        // Mettre à jour les quantités de produits
        for (const item of products) {
            const produit = await Produit.findById(item.produitId);
            produit.stockQuantity -= item.stockQuantity;
            await produit.save();
        }

        // Envoi de l'e-mail de confirmation
        const produitsDetails = await Produit.find({ '_id': { $in: products.map(item => item.produitId) } });
        const productList = produitsDetails.map(produit => `${produit.name} (Ref: ${produit.reference})`).join(', ');
...
async function generatePDF(commande, produitsDetails) {
    return new Promise((resolve, reject) => {
        ...
        produitsDetails.forEach((produit, index) => {
            const item = commande.products.find(p => p.produitId.toString() === produit._id.toString());
            doc.fontSize(12)
                .fillColor('#000000')
                .text(`${index + 1}`, 50, itemY)
                .text(`${produit.nom}`, 70, itemY)
                .text(`${produit.reference}`, 250, itemY)
                .text(`${item.stockQuantity}`, 350, itemY)
                .text(`${produit.price.toFixed(2)} DT`, 450, itemY, { width: 90, align: 'right' });
            itemY += 20;
        });
...
exports.getAllCommandes = async (req, res) => {
    try {
        const commandes = await Commande.find().populate('products.produitId');
        res.json(commandes);
    } catch (error) {
...
exports.getCommandeById = async (req, res) => {
    try {
        const commande = await Commande.findById(req.params.id).populate('products.produitId');
        if (!commande) {
...
exports.annulerCommande = async (req, res) => {
    try {
        ...
        for (const item of commande.products) {
            const produit = await Produit.findById(item.produitId);
            produitsQuantitesAvantAnnulation[item.produitId] = produit.stockQuantity;
        }
